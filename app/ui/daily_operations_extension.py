...
from app.ui.daily_operations import DailyOperationsDialog

_INSTALLED = False
_ORIGINAL_NAVIGATE = None


def install_daily_operations_extension() -> None:
    ...
    def _navigate_with_report(self, row: int) -> None:
        ...
        if (
            ...
            QMessageBox.warning(self, "Informe operativo", "El usuario actual no tiene permiso para ver el informe operativo.")
            return
        DailyOperationsDialog(self).exec_()

    FemagDesktopWindow._navigate = _navigate_with_report
    _INSTALLED = True


def uninstall_daily_operations_extension() -> None:
    global _INSTALLED, _ORIGINAL_NAVIGATE
    if not _INSTALLED:
        return
    from app.ui.desktop_app import FemagDesktopWindow
