...
class FemagDesktopWindow(QMainWindow):
    ...
    def _build(self) -> None:
        ...
        self._add_page(
            ...
            self._master_abm_page(
                "Choferes",
                ["Nombre", "Transportista", "Disponible"],
                _driver_rows,
                DriverEntryDialog,
                "newDriverButton",
                "editDriverButton",
    ...
    def _master_abm_page(
        ...
        page = _page(title, "ABM mínimo para operar Órdenes de carga")
        layout = page.layout()
        feedback = QLabel("")
        feedback.setObjectName(f"{new_button_name}Feedback")
        actions = QHBoxLayout()
        ...
        table.setObjectName(f"{new_button_name}Table")
        table.setHorizontalHeaderLabels(columns)
        ...
        def open_edit() -> None:
            ...
            if row_id is None:
                ...
                return
            dialog = dialog_class(current_user=self.shell.username, record_id=row_id, parent=self)
            if dialog.exec_() == QDialog.Accepted:
                refresh()
...
class ClientEntryDialog(QDialog):
    ...
    def _build(self) -> None:
        ...
        form.addWidget(QLabel("Teléfono"), 3, 0)
...
class ClientAddressEntryDialog(QDialog):
    ...
    def _build(self) -> None:
        ...
        form.addWidget(QLabel("Dirección"), 4, 0)
    ...
    def _save(self) -> None:
        ...
        if client_id is None or not province or not city or not street:
            self.feedback.setText("Complete cliente, provincia, ciudad y dirección.")
            return
        try:
...
class CarrierEntryDialog(QDialog):
    ...
    def _build(self) -> None:
        ...
        form.addWidget(QLabel("CUIT"), 1, 0)
        form.addWidget(self.cuit_input, 1, 1)
        form.addWidget(QLabel("Teléfono"), 2, 0)
...
class TruckEntryDialog(QDialog):
    def __init__(self, *, current_user: str, record_id: int | None = None, parent=None):
        ...
        self.setWindowTitle("Camión")
        self._build()
        self._load_record()

    def _build(self) -> None:
        layout = _entry_layout(self, "Camión / patente")
        form = QGridLayout()
...
class ProductEntryDialog(QDialog):
    ...
    def _build(self) -> None:
        layout = _entry_layout(self, "Producto / presentación")
        form = QGridLayout()
        self.name_input = QLineEdit()
