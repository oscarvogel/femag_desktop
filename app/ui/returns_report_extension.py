...
def install_returns_report_extension() -> None:
    ...
    def _navigate_with_report(self, row: int) -> None:
        ...
        if (
            ...
            QMessageBox.warning(self, "Devoluciones", "El usuario actual no tiene permiso para ver las devoluciones.")
            return
        ReturnsReportDialog(self).exec_()

    FemagDesktopWindow._navigate = _navigate_with_report
    _INSTALLED = True


def uninstall_returns_report_extension() -> None:
    global _INSTALLED, _ORIGINAL_NAVIGATE
    if not _INSTALLED:
        return
    from app.ui.desktop_app import FemagDesktopWindow
