...
from app.ui.managerial_sales_dispatch import ManagerialSalesDispatchDialog
...
def install_managerial_sales_dispatch_extension() -> None:
    ...
    def _navigate_with_report(self, row: int) -> None:
        ...
        if route != "managerial_sales_dispatch":
        ...
        if not PermissionService().can_view_managerial_dashboard(self.user):
            ...
            return
        ManagerialSalesDispatchDialog(self).exec_()

    FemagDesktopWindow._navigate = _navigate_with_report
    _INSTALLED = True


def uninstall_managerial_sales_dispatch_extension() -> None:
    global _INSTALLED, _ORIGINAL_NAVIGATE
    if not _INSTALLED:
        return
