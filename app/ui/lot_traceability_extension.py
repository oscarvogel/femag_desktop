...
def install_lot_traceability_extension() -> None:
    ...
    def _navigate_with_report(self, row: int) -> None:
        ...
        if (
            ...
            QMessageBox.warning(self, "Trazabilidad", "El usuario actual no tiene permiso para ver la trazabilidad.")
            return
        LotTraceabilityDialog(self).exec_()

    FemagDesktopWindow._navigate = _navigate_with_report
    _INSTALLED = True


def uninstall_lot_traceability_extension() -> None:
    global _INSTALLED, _ORIGINAL_NAVIGATE
    if not _INSTALLED:
        return
    from app.ui.desktop_app import FemagDesktopWindow
