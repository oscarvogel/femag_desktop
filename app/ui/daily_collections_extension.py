...
from app.ui.daily_collections import DailyCollectionsDialog
...
def install_daily_collections_extension() -> None:
    ...
    def _navigate_with_report(self, row: int) -> None:
        ...
        if not PermissionService().can_view_managerial_dashboard(self.user):
            ...
            return
        DailyCollectionsDialog(self).exec_()

    FemagDesktopWindow._navigate = _navigate_with_report
    _INSTALLED = True


def uninstall_daily_collections_extension() -> None:
    global _INSTALLED, _ORIGINAL_NAVIGATE
    if not _INSTALLED:
        return
