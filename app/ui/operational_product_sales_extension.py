...
from app.ui.operational_product_sales import OperationalProductSalesDialog

_INSTALLED = False
_ORIGINAL_NAVIGATE = None


def install_operational_product_sales_extension() -> None:
    ...
    def _navigate_with_report(self, row: int) -> None:
        ...
        if route != "operational_product_sales":
        ...
        OperationalProductSalesDialog(self).exec_()

    FemagDesktopWindow._navigate = _navigate_with_report
    _INSTALLED = True


def uninstall_operational_product_sales_extension() -> None:
    global _INSTALLED, _ORIGINAL_NAVIGATE
    if not _INSTALLED:
        return
