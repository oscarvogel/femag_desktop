...
function drawCharts(d){destroyCharts();const wrap=document.getElementById('clientsChartWrap');if(typeof Chart==='undefined'){document.getElementById('chartNotice').style.display='block';wrap.style.display='none';return}document.getElementById('chartNotice').style.display='none';wrap.style.display='block';const common={responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{grid:{display:false}},y:{beginAtZero:true,grid:{color:'#edf1f6'}}}};charts.push(new Chart(document.getElementById('evolutionChart'),{type:'line',data:{labels:d.monthly_evolution.map(x=>x.label),datasets:[{data:d.monthly_evolution.map(x=>x.total),borderColor:'#1559d6',backgroundColor:'rgba(21,89,214,.10)',fill:true,tension:.32,pointRadius:3}]},options:common}));charts.push(new Chart(document.getElementById('clientsChart'),{type:'bar',data:{labels:d.top_clients.map(x=>x.name),datasets:[{data:d.top_clients.map(x=>x.total),backgroundColor:'#1559d6',borderRadius:7}]},options:{...common,indexAxis:'y'}}))}
