...
def test_generate_persists_batch_snapshot_audit_and_file(db, tmp_path):
    ...
    assert audit.record_ref == f"F150Batch:{batch.id}"
...
def test_filters_by_date_client_number_status_and_inclusion(db, tmp_path):
    remittance = _issued_remittance()
    service = F150BatchService("admin")

    rows = service.eligible_remittances(
    ...
    assert [row.id for row in rows] == [remittance.id]

    service.generate([remittance], tmp_path / "batch.TXT")

    assert service.eligible_remittances(included=False) == []
    assert [row.id for row in service.eligible_remittances(included=True)] == [remittance.id]
